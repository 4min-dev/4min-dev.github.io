...
const boardProfileContainer = document.querySelector('.users__container')
...
function createUser(user, index) {
    ...
    userCard.classList.add('leaderboard__card', 'flex', 'align__center', 'justify__space__between')

    userCard.innerHTML = `
      <div class="leaderboard__card__content flex align__center">
                        <div class="leaderboard__user__avatar">
                            <img src=${user.avatar_url} alt="User avatar" class="leaderboard__user__avatar__value"/>
                        </div>

                        <div class="leaderboard__card__text__container flex column">
                            <span class="leaderboard__username">${user.first_name}</span>
                            <div class="leaderboard__summ__container flex align__end">
                                ...
                                <span class="leaderboard__user__summ__value">${getFormattedTokens(user.tokens)}</span>
                            </div>
                        </div>
                    </div>

                    <span class="leaderboard__card__number">#${index + 1}</span>
    `

    boardProfileContainer.appendChild(userCard)
}
...
async function getProfileData() {
    ...
    try {
        ...
        const filteredUsersData = usersData.filter((user) => user.tg_id !== userData.tg_id)
        ...
        const userIndex = usersData.findIndex(user => user.tg_id === userData.tg_id)

        userLevel.textContent = `${level} Уровень`
        userFullname.textContent = username
        profileUsername.innerHTML = `<span class="opacity">@</span>${telegramUsername}`
        ...
        console.log(boardProfileContainer)
        boardProfileContainer.innerHTML = `
        <div class="leaderboard__card my__profile flex align__center justify__space__between">
                    <div class="leaderboard__card__content flex align__center">
                        <div class="leaderboard__user__avatar">
                            <img src=${avatar} alt="User avatar" class="leaderboard__user__avatar__value"/>
                        </div>
                        ...
                        <div class="leaderboard__card__text__container flex column">
                            ...
                            <div class="leaderboard__summ__container flex align__end">
                                ...
                                <span class="leaderboard__user__summ__value">${formattedTokens}</span>
                            </div>
                        </div>
                    </div>

                    <span class="leaderboard__card__number">#${userIndex + 1}</span>
                </div>
        `
        await filteredUsersData.map((user, index) => createUser(user, index))
