document.addEventListener('DOMContentLoaded', () => {
    ...
    function processStoreCards(cards) {
        cards.forEach(card => {
        ...
        mutations.forEach(mutation => {
            if (mutation.addedNodes.length > 0) {
                const newCards = Array.from(mutation.addedNodes)
                    .filter(node => node.nodeType === Node.ELEMENT_NODE && node.matches('.store__card'))
                    .concat(
                        Array.from(mutation.target.querySelectorAll('.store__card'))
                    )

                processStoreCards(newCards)
            }
        })
    })

    const storeContainer = document.querySelector('.store__cards__container')
    if (storeContainer) {
        observer.observe(storeContainer, { childList: true, subtree: true })
    }

    const existingCards = document.querySelectorAll('.store__card')
    processStoreCards(existingCards)
})
